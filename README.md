...
## LLM verifier - Used to find a bias in AI activity
Findings from this project. 
AI seems to have a bias towards the user, and when provided with code different from the one 
generated by the model, it often selects the user’s code. The inherited bias requires the 
user to use multiple AI models for a given task to maintain objectivity and gather useful 
information. Using ChatGPT as a verifier allowed for direct comparison between the two 
results, without any bias, and led to my second point. The AI model used as a Comparator 
provides you with raw information and direct comparison, with a summary, but allows you 
to decide based on what differences are highlighted. 
